...
func Register{{ $svcName }}MCPHandler(s *mcpsdk.Server, srv {{ $svcName }}MCPServer, opts ...mcp.Option) {
...
{{- if and $tool.MethodOpts $tool.MethodOpts.Elicitation }}
			...
			// body runs twice.
			elicitResult, elicitPending, elicitErr := mcp.RunElicitation(req, "{{ $tool.MethodOpts.Elicitation.Message }}", elicitFields)
			if elicitErr != nil {
...
{{- if and $tool.MethodOpts $tool.MethodOpts.Prompt }}
	...
	}, mcp.DefaultPromptHandler("{{ escapeQuotes $tool.MethodOpts.Prompt.Description }}"))
{{- end }}
{{- end }}
{{- end }}
{{- if and $svcOpts $svcOpts.App }}
	...
	s.AddResource(&mcpsdk.Resource{
		...
		MIMEType: "text/html",
	}, mcp.DefaultAppResourceHandler("{{ escapeQuotes $svcOpts.App.Name }}", "{{ $svcOpts.App.Version }}", "{{ escapeQuotes $svcOpts.App.Description }}"))
{{- end }}
}
{{- end }}
...
func ForwardTo{{ $svcName }}MCPClient(s *mcpsdk.Server, client {{ $svcName }}MCPClient, opts ...mcp.Option) {
	cfg := mcp.ApplyOptions(opts...)
	_ = cfg
{{- if and $svcOpts $svcOpts.App }}
	appResourceURI := mcp.AppResourceURI("{{ $svcName }}")
