...
var (
	TodoService_CreateTodoTool = mcp.MustCreateTool("todo_service-create_todo_v1", `Creates a new todo item under a user. Requires parent (e.g. users/alice), a todo object with title/description/priority, and a unique todo_id.`, TodoService_CreateTodoSchemaJSON)
	TodoService_DeleteTodoTool = mcp.MustCreateTool("todo_service-delete_todo_v1", `Permanently deletes a todo item by its resource name. This action cannot be undone.`, TodoService_DeleteTodoSchemaJSON)
	TodoService_GetTodoTool    = mcp.MustCreateTool("todo_service-get_todo_v1", `Retrieves a single todo item by its resource name (e.g. users/alice/todos/abc123).`, TodoService_GetTodoSchemaJSON)
	TodoService_ListTodosTool  = mcp.MustCreateTool("todo_service-list_todos_v1", `Lists all todo items for a user. Supports pagination via page_size and page_token.`, TodoService_ListTodosSchemaJSON)
	TodoService_UpdateTodoTool = mcp.MustCreateTool("todo_service-update_todo_v1", `Updates an existing todo item. Send the todo with its resource name and the fields to update. Use update_mask to specify which fields to modify.`, TodoService_UpdateTodoSchemaJSON)
)
...
func RegisterTodoServiceMCPHandler(s *mcpsdk.Server, srv TodoServiceMCPServer, opts ...mcp.Option) {
	...
	}, mcp.DefaultPromptHandler("Summarize all pending todo items for a user"))
	s.AddPrompt(&mcpsdk.Prompt{
	...
	}, mcp.DefaultPromptHandler("Suggest a priority ordering for a user's incomplete todos"))

	s.AddResource(&mcpsdk.Resource{
		URI:      appResourceURI,
		Name:     "Todo App",
		MIMEType: "text/html",
	}, mcp.DefaultAppResourceHandler("Todo App", "1.0.0", "A simple todo management application"))
}
...
func ForwardToTodoServiceMCPClient(s *mcpsdk.Server, client TodoServiceMCPClient, opts ...mcp.Option) {
	cfg := mcp.ApplyOptions(opts...)
	_ = cfg
	appResourceURI := mcp.AppResourceURI("TodoService")
	{
